...
# Usage example
data <- fromJSON(r"{C:\<your-path>\Ch12 - Calculating Columns Using Complex Algorithms, Distances\email-addresses-to-dedup.json}")


deduplicated_data_jaccard <- deduplicate_data(data, similarity_method = "jaccard", similarity_threshold = 0.874)

deduplicated_data_jaccard[[1]] # deduplicated_data
deduplicated_data_jaccard[[2]] # duplicated_data



deduplicated_data_jaro_winkler <- deduplicate_data(data, similarity_method = "jw", p = 0.1, similarity_threshold = 0.872)

deduplicated_data_jaro_winkler[[1]] # deduplicated_data
deduplicated_data_jaro_winkler[[2]] # duplicated_data



deduplicated_data_jaccard_3grams <- deduplicate_data(data, similarity_method = "jaccard", q = 3, similarity_threshold = 0.55)

deduplicated_data_jaccard_3grams[[1]] # deduplicated_data
deduplicated_data_jaccard_3grams[[2]] # duplicated_data



deduplicated_data_jaro_winkler_3grams <- deduplicate_data(data, similarity_method = "qgram", q = 3, similarity_threshold = 0.71)

deduplicated_data_jaro_winkler_3grams[[1]] # deduplicated_data
deduplicated_data_jaro_winkler_3grams[[2]] # duplicated_data
