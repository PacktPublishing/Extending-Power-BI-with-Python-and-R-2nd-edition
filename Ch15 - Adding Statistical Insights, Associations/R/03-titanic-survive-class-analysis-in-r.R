...
calc_corr <- function(data, row_name, col_name, numeric_replace_value = 0, theil_uncert=TRUE) {
  ...
  if (row_data_type == 'numeric' & col_data_type == 'numeric') {
    ...
    return((c %>% corrr::correlate(method = 'pearson', quiet = T))[[1,3]])
  }
  ...
  if (row_data_type == 'numeric' & (col_data_type == 'character' | col_data_type == 'factor')) {
    return(correlation_ratio(categories = col_vec, measurements = row_vec, numeric_replace_value = 0))
  }
  
  if ((row_data_type == 'character' | row_data_type == 'factor') & col_data_type == 'numeric') {
    return(correlation_ratio(categories = row_vec, measurements = col_vec, numeric_replace_value = 0))
  }
  
  if (theil_uncert) {
    return(DescTools::UncertCoef(row_vec, col_vec, direction = 'row'))
  } else {
    return(rstatix::cramer_v(x=row_vec, y=col_vec))
  }
}
...
# Transform categorical variables in factor columns
tbl <- tbl %>% 
